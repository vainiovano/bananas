...
// modules/objdetect/src/aruco/aruco_detector.cpp from the OpenCV project.
...
static void _threshold(const cuda::GpuMat &_in, cuda::GpuMat &_out, int winSize,
    ...
    CHECK_NPP(nppiFilterThresholdAdaptiveBoxBorder_8u_C1R_Ctx(
        _in.data, _in.step, {_in.cols, _in.rows}, {0, 0}, _out.data, _out.step,
        {_out.cols, _out.rows}, {winSize, winSize}, constant, 0, 255,
        NPP_BORDER_REPLICATE, npp_stream_context));
}
...
static void _detectInitialCandidates(
    ...
    CV_Assert(params.adaptiveThreshWinSizeMax >=
    ...
    for (int i = 0; i < nScales; i++) {
        int currScale = params.adaptiveThreshWinSizeMin +
                        i * params.adaptiveThreshWinSizeStep;
        auto stream_context = npp_stream_context;
        stream_context.hStream = cuda::StreamAccessor::getStream(streams[i]);
        _threshold(grey, thresholded[i], currScale,
                   params.adaptiveThreshConstant, stream_context);
    }
    ...
    parallel_for_(Range(0, nScales), [&](const Range& range) {
        ...
        for (int i = begin; i < end; i++) {
            ...
            // detect rectangles
            _findMarkerContours(
                thresholded_cpu[i].createMatHeader(), candidatesArrays[i],
                contoursArrays[i], params.minMarkerPerimeterRate,
...
struct ArucoDetector::ArucoDetectorImpl {
    ...
    /// fxfy parameter from paper [1] Speeded up detection of squared fiducial
    /// markers, 2018, FJ Romera-Ramirez et al.
    float fxfy;
    ...
    /// NPP stream context. The hStream value needs to be set to a suitable
    /// stream before usage.
    NppStreamContext npp_stream_context;

    ArucoDetectorImpl() {}

    ArucoDetectorImpl(const Dictionary& _dictionary,
                      const DetectorParameters& _detectorParams,
                      const RefineParameters& _refineParams, Size _imageSize)
        : dictionary(_dictionary), detectorParams(_detectorParams),
          refineParams(_refineParams), imageSize(_imageSize),
          /// equation (2) from paper [1]
          fxfy(
              (!detectorParams.useAruco3Detection
                   ? 1.f
                   : detectorParams.minSideLengthCanonicalImg /
                         (detectorParams.minSideLengthCanonicalImg +
                          std::max(_imageSize.width, _imageSize.height) *
...
ArucoDetector::ArucoDetector(const Dictionary& _dictionary,
    ...
    arucoDetectorImpl = makePtr<ArucoDetectorImpl>(_dictionary, _detectorParams,
                                                   _refineParams, _imageSize);
}
...
void ArucoDetector::detectMarkers(InputArray _image, OutputArrayOfArrays _corners, OutputArray _ids,
    ...
    const float fxfy = arucoDetectorImpl->fxfy;
    ...
    Mat original_grey;
    if (fxfy == 1.f) {
        original_grey = arucoDetectorImpl->grey_cpu.createMatHeader();
    }
    _convertToGrey(_image, original_grey);

    // Aruco3 functionality is the extension of Aruco.
    // The description can be found in [1].
    ...
    if (detectorParams.useAruco3Detection) {
        ...
        const float img_area = static_cast<float>(original_grey.rows*original_grey.cols);
    ...
    buildPyramid(original_grey, grey_pyramid, num_levels);

    // resize to segmentation image
    // in this reduces size the contours will be detected
    auto grey = arucoDetectorImpl->grey_cpu.createMatHeader();
    if (fxfy != 1.f)
        resize(original_grey, grey, arucoDetectorImpl->downsampledSize);
